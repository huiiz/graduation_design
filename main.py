...
class ImageItem(ft.UserControl):
    ...
    def click(self, e: ft.control_event.ControlEvent):
        ...
        :return: None
        """
        if self.parent.selected_image_item:
            if id(self.parent.selected_image_item) == id(self):
                return
            self.parent.selected_image_item.selected = False
            self.parent.selected_image_item.set_cancel_selected_status()
        self.selected = True
        self.parent.selected_image_item = self
        self.set_selected_status()
...
class ResultPanel(ft.UserControl):
    def __init__(self, parent):
        ...
        self.single_control_panel = ft.Column(
            controls=[
                ft.Text(value='这里将要显示单图像结果与操作')
            ],
            alignment=ft.MainAxisAlignment.CENTER
        )
        self.all_control_panel = ft.Column(
            controls=[
                ft.Text(value='这里将要显示总体结果')
            ],
            alignment=ft.MainAxisAlignment.CENTER
        )
...
class CuttingErrorApp(ft.UserControl):
    def __init__(self, width: float, height: float):
        ...
        self.main_content = ft.Container(                                       # 2.中间显示图片目标检测内容
        ...
        self.set_thumb_ls_height()                                              # 设置缩略图显示列表
    ...
    def set_thumb_ls_height(self):
        """
        根据窗口大小，自适应设置缩略图列表的高度
        :return: None
        """
        self.thumb_img_ls.height = self.height - 120                            # 设置缩略图大小，将窗口大小减去一个值
